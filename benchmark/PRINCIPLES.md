...
集中维护在根目录 [COMPATIBILITY.zh-CN.md](../COMPATIBILITY.zh-CN.md)
（该文档非免检权威——变更时须与上游来源交叉比对，见其第 5 节维护
规则；本节只保留读报告所需的最小事实，以交叉比对后的结论为准）。
要点（2026-08 核对）：
...
- MindIE 全版本（≤ 3.1.0）：无 salt、无 release、无 agent_hint；其
  ...
  组合据报会被 HTTP 501 主动拒绝**（agent-core 联调经验，本仓库未独立
  复核——本项目 2026-08-20/24 真机的 501 已勘误为客户端 role 序列化
  缺陷，详见 COMPATIBILITY 勘误记录），客户端自动降级
...
**对照有效性守卫（⛔）**：一切判定的前提是配对双方跑过**同一份工作
负载**。若两侧任务完成集不一致、或 LLM 调用数差异过大（< 70%），两侧
样本不可比——例如亲和侧工具任务早夭时，幸存的短调用会伪造出"E2E ✅"
（2026-08-24 报告的 E2E -39.5% 即此假象）。此时化验单保留原始数字但把
全部判定强制为 ➖ 并给出"对照无效"警报，防止幸存者偏差冒充结论。

**KV 命中率 ➖ 的含义**：引擎/网关未返回 `prompt_tokens_details.
cached_tokens` 时（实测多见），客户端拿不到真实命中数据——报告显示
➖（数据不可得），而非误导性的 0.0%；此时核心四项仅三项可判，需网关
透传 usage 详情或配置 `--metrics-url` 走引擎侧指标。
